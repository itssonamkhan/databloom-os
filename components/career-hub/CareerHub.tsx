...
export default function CareerHub() {
  ...
  function toggleCompletion(id: string) {
    ...
    if (result.xpAward) {
      addXP({ rewardId: `career:completion:${id}`, source: "career", optimisticXP: result.xpAward });
  ...
  function awardCertificationXP(amount: number, certificationId: string) {
    if (!amount) return;
    const rewardKind = amount === 70 ? "added-completed" : amount === 50 ? "completed" : "added";
    addXP({ rewardId: `career:certification:${rewardKind}:${certificationId}`, source: "career-certification", optimisticXP: amount });
      ...
      <div className="min-w-0 space-y-7 text-slate-950">
        <header className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-gradient-to-br from-purple-100 via-pink-100 to-sky-100 p-7 shadow-lg sm:p-10">
          ...
          <div className="relative flex min-w-0 flex-col gap-7 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div className="grid min-w-0 grid-cols-2 gap-3 rounded-3xl border border-white bg-white/75 p-4 shadow-sm sm:grid-cols-4">
...
function CareerHome({ preferences, state, summary, recommended, navigate }: {
  ...
  return <div className="min-w-0 space-y-7">
    <section className="grid min-w-0 gap-6 xl:grid-cols-[1.2fr_0.8fr]">
      <article className="min-w-0 rounded-[2rem] border border-purple-200 bg-gradient-to-br from-white via-purple-50 to-pink-50 p-7 shadow-lg">
        <p className="text-sm font-black uppercase tracking-[0.16em] text-purple-700">Welcome, {name}</p>
        <h2 className="mt-2 break-words text-3xl font-black">Your goal: {preferences.careerGoal}</h2>
      ...
      <article className="min-w-0 rounded-[2rem] border border-amber-200 bg-amber-50/80 p-7 shadow-lg">
    ...
    <section>
      ...
      <div className="mt-5 grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <QuickLink icon="🧾" label="Resume Builder" href="/resume-builder" />
        <QuickLink icon="🏆" label="Portfolio Project Studio" href="/portfolio-project-studio" />
...
function Roadmaps({ roadmap, roadmapId, setRoadmapId, completedIds, onToggle }: { roadmap: (typeof careerRoadmaps)[number]; roadmapId: string; setRoadmapId: (id: string) => void; completedIds: string[]; onToggle: (id: string) => void }) {
  ...
  return <section className="min-w-0">
    ...
    <div className="mt-5 flex flex-wrap gap-2">{careerRoadmaps.map((item) => <button key={item.id} type="button" aria-pressed={roadmapId === item.id} onClick={() => { playClickSound(); setRoadmapId(item.id); }} className={`rounded-2xl px-4 py-3 font-black ${roadmapId === item.id ? "bg-purple-700 text-white" : "border border-purple-200 bg-white text-purple-800"}`}>{item.icon} {item.title}</button>)}</div>
    <article className="mt-6 min-w-0 rounded-[2rem] border border-purple-100 bg-white/85 p-6 shadow-lg sm:p-8">
      ...
      <ol className="mt-6 grid min-w-0 gap-4 lg:grid-cols-2">{roadmap.steps.map((step, index) => { const checked = completedIds.includes(step.id); return <li key={step.id} className={`min-w-0 rounded-3xl border p-5 ${checked ? "border-emerald-200 bg-emerald-50" : "border-purple-100 bg-purple-50/40"}`}><label className="flex min-w-0 cursor-pointer items-start gap-4"><input type="checkbox" checked={checked} onChange={() => onToggle(step.id)} className="mt-1 size-5 accent-emerald-600" /><span className="min-w-0"><span className="break-words text-sm font-black uppercase tracking-wider text-purple-700">{index + 1}. {step.stage}</span><span className="mt-1 block break-words text-xl font-black">{step.title}</span><span className="mt-2 block break-words leading-7 text-slate-700">{step.detail}</span><span className="mt-2 block text-sm font-black text-amber-700">+10 XP first completion</span></span></label></li>; })}</ol>
    </article>
  </section>;
}
...
function GuideView({ guide, completedIds, onToggle }: { guide: Guide; completedIds: string[]; onToggle: (id: string) => void }) {
  ...
  return <section className="min-w-0 space-y-6">
    <header className="rounded-[2rem] border border-white bg-gradient-to-br from-purple-100 via-white to-pink-100 p-7 shadow-lg"><p className="text-5xl">{guide.icon}</p><h1 className="mt-3 text-4xl font-black">{guide.title}</h1><p className="mt-3 max-w-3xl leading-7 text-slate-700">{guide.intro}</p>{guide.id === "resume" ? <Link href="/resume-builder" onClick={playClickSound} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-2xl bg-purple-700 px-5 font-black text-white">Open Resume Builder <ArrowRight size={18} /></Link> : null}{guide.id === "portfolio" ? <Link href="/portfolio-project-studio" onClick={playClickSound} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-2xl bg-purple-700 px-5 font-black text-white">Open Portfolio Project Studio <ArrowRight size={18} /></Link> : null}<div className="mt-5 flex items-center gap-4"><div className="h-3 flex-1 overflow-hidden rounded-full bg-purple-100"><div className="h-full bg-gradient-to-r from-purple-600 to-pink-500" style={{ width: `${score}%` }} /></div><span className="font-black text-purple-800">{score}% ready</span></div></header>
    <div className="grid min-w-0 gap-6 xl:grid-cols-[0.85fr_1.15fr]">
      <article className="min-w-0 rounded-[2rem] border border-emerald-100 bg-white/85 p-6 shadow-lg"><h2 className="text-2xl font-black">Readiness checklist</h2><p className="mt-2 text-sm text-slate-600">Each item awards 10 XP once.</p><div className="mt-5 space-y-3">{guide.checklist.map((item) => { const checked = completedIds.includes(item.id); return <label key={item.id} className={`flex min-w-0 cursor-pointer items-start gap-3 rounded-2xl border p-4 ${checked ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-white"}`}><input type="checkbox" checked={checked} onChange={() => onToggle(item.id)} className="mt-1 size-4 accent-emerald-600" /><span className={`min-w-0 break-words font-semibold leading-6 ${checked ? "text-slate-500 line-through" : ""}`}>{item.label}</span></label>; })}</div></article>
      <div className="grid min-w-0 gap-4 md:grid-cols-2">{guide.sections.map((section) => <article key={section.title} className="min-w-0 rounded-3xl border border-purple-100 bg-white p-5 shadow-sm"><h2 className="break-words text-xl font-black text-purple-900">{section.title}</h2><ul className="mt-4 list-disc space-y-2 break-words pl-5 text-slate-700">{section.points.map((point) => <li key={point} className="leading-6">{point}</li>)}</ul></article>)}</div>
    </div>
  </section>;
}
...
function ApplicationTracker({ kind, state, onSync }: { kind: ApplicationKind; state: CareerHubState; onSync: () => void }) {
  ...
  return <section className="min-w-0 space-y-6">
    ...
    <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-4"><ProgressCard icon="📬" label="Total" value={String(records.length)} /><ProgressCard icon="📅" label="Upcoming follow-ups" value={String(upcoming)} /><ProgressCard icon="⚠️" label="Overdue follow-ups" value={String(overdue)} /><ProgressCard icon="🎉" label="Offers" value={String(records.filter((item) => item.status === "Offer").length)} /></div>
    ...
    <div className="grid min-w-0 gap-3 rounded-3xl border border-purple-100 bg-white/80 p-4 shadow-sm md:grid-cols-[1fr_0.55fr_0.55fr]"><label className="relative min-w-0"><span className="sr-only">Search applications</span><Search className="absolute left-3 top-3.5 text-slate-500" size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search company, role, location, or notes" className="min-h-12 w-full min-w-0 rounded-2xl border border-slate-300 pl-10 pr-4" /></label><LabeledSelect label="Filter status" value={status} onChange={setStatus} options={["All", ...statuses]} /><LabeledSelect label="Sort applications" value={sort} onChange={setSort} options={["updated", "company", "followup"]} labels={{ updated: "Recently updated", company: "Company A–Z", followup: "Follow-up date" }} /></div>
...
function ApplicationForm({ form, setForm, statuses, editing, onClose, onSubmit }: { form: ApplicationInput; setForm: React.Dispatch<React.SetStateAction<ApplicationInput>>; statuses: readonly string[]; editing: boolean; onClose: () => void; onSubmit: (event: React.FormEvent) => void }) {
  ...
  return <form onSubmit={onSubmit} className="min-w-0 rounded-[2rem] border border-purple-200 bg-purple-50/70 p-5 shadow-lg sm:p-7"><div className="flex min-w-0 items-center justify-between gap-4"><h2 className="min-w-0 break-words text-2xl font-black">{editing ? "Edit application" : "Add application"}</h2><button type="button" onClick={onClose} aria-label="Close application form" className="grid size-11 shrink-0 place-items-center rounded-xl bg-white text-slate-700"><X size={19} /></button></div><div className="mt-5 grid min-w-0 gap-4 md:grid-cols-2 xl:grid-cols-3">
...
function ApplicationCard({ item, today, onEdit, onDelete }: { item: ApplicationRecord; today: string; onEdit: () => void; onDelete: () => void }) {
  ...
  return <article className="min-w-0 rounded-3xl border border-purple-100 bg-white p-5 shadow-md"><div className="flex min-w-0 items-start justify-between gap-4"><div className="min-w-0"><span className="inline-block max-w-full break-words rounded-full bg-purple-100 px-3 py-1 text-xs font-black text-purple-800">{item.status}</span><h2 className="mt-3 break-words text-2xl font-black">{item.company}</h2><p className="mt-1 break-words font-bold text-slate-700">{item.role}</p></div><div className="flex shrink-0 gap-2"><button type="button" onClick={onEdit} aria-label={`Edit ${item.company} ${item.role}`} className="grid size-11 place-items-center rounded-xl bg-blue-50 text-blue-700"><Edit3 size={17} /></button><button type="button" onClick={onDelete} aria-label={`Delete ${item.company} ${item.role}`} className="grid size-11 place-items-center rounded-xl bg-rose-50 text-rose-700"><Trash2 size={17} /></button></div></div><dl className="mt-4 grid min-w-0 grid-cols-2 gap-3 text-sm"><Info label="Work type" value={item.workType || "Not set"} /><Info label="Location" value={item.location || "Not set"} /><Info label="Applied" value={item.dateApplied || "Not set"} /><Info label="Deadline" value={item.deadline || "Not set"} /><Info label="Interview stage" value={item.interviewStage || "Not set"} /><Info label="Follow-up" value={item.followUpDate || "Not set"} alert={Boolean(overdue)} /></dl>{item.notes ? <p className="mt-4 break-words rounded-2xl bg-slate-50 p-3 text-sm leading-6 text-slate-700">{item.notes}</p> : null}{item.applicationLink ? <a href={item.applicationLink} target="_blank" rel="noreferrer" className="mt-4 inline-flex max-w-full items-center gap-2 break-words font-black text-purple-800">Open application <ExternalLink className="shrink-0" size={16} /></a> : null}</article>;
}
...
function CompanyExplorer({ query, setQuery, companiesToShow, favorites }: { query: string; setQuery: (value: string) => void; companiesToShow: typeof companies; favorites: string[] }) {
  return <section className="min-w-0"><SectionHeading eyebrow="Company Explorer" title="Prepare for target companies" description="Stable preparation guides only—no live openings, salary claims, or changing hiring-policy promises." /><label className="relative mt-5 block max-w-xl min-w-0"><span className="sr-only">Search companies</span><Search className="absolute left-4 top-3.5 text-slate-500" size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search company or industry" className="min-h-12 w-full min-w-0 rounded-2xl border border-purple-200 bg-white pl-11 pr-4" /></label><div className="mt-6 grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{companiesToShow.map((company) => <Link key={company.slug} href={`/career-hub/company/${company.slug}`} onClick={playClickSound} className="group min-w-0 rounded-3xl border border-purple-100 bg-gradient-to-br from-white to-purple-50 p-5 shadow-md transition hover:-translate-y-1 hover:shadow-xl"><div className="flex items-center justify-between"><span className="text-3xl">🏢</span>{favorites.includes(company.slug) ? <Heart className="text-pink-600" size={19} fill="currentColor" /> : null}</div><h2 className="mt-4 break-words text-2xl font-black">{company.name}</h2><p className="mt-2 break-words text-sm font-semibold text-slate-600">{company.type}</p><span className="mt-5 inline-flex items-center gap-2 font-black text-purple-800">Prepare <ArrowRight className="transition group-hover:translate-x-1" size={17} /></span></Link>)}</div>{companiesToShow.length === 0 ? <EmptyState icon="🔎" text="No companies match this search." /> : null}</section>;
}
...
function CertificationTracker({ state, onSync, onXPAward }: { state: CareerHubState; onSync: () => void; onXPAward: (amount: number, certificationId: string) => void }) {
  ...
  function submit(event: React.FormEvent) { event.preventDefault(); const result = saveCertification({ ...form, name: form.name.trim() }, editing?.id); if (result.saved) { onXPAward(result.xpAward, result.item.id); playSuccessSound(); setForm(blankCertification); setEditing(null); onSync(); } }
  ...
  return <section className="min-w-0 space-y-6"><SectionHeading eyebrow="Certification Tracker" title="Track credentials with purpose" description="Save plans and evidence without treating certificates as a substitute for project skill." /><Link href="/certification-hub" onClick={playClickSound} className="inline-flex min-h-12 max-w-full items-center gap-2 rounded-2xl bg-purple-700 px-5 font-black text-white">Open the full Certification Hub <ArrowRight className="shrink-0" size={17} /></Link><form onSubmit={submit} className="min-w-0 rounded-[2rem] border border-purple-200 bg-white/85 p-6 shadow-lg"><div className="flex min-w-0 items-center justify-between gap-4"><h2 className="min-w-0 break-words text-2xl font-black">{editing ? "Edit certification" : "Add certification"}</h2>{editing ? <button type="button" onClick={() => { setEditing(null); setForm(blankCertification); }} aria-label="Cancel certification editing" className="grid size-11 shrink-0 place-items-center rounded-xl bg-slate-100"><X size={18} /></button> : null}</div><div className="mt-5 grid min-w-0 gap-4 md:grid-cols-2 lg:grid-cols-3"><Field label="Certification name" required value={form.name} onChange={(value) => update("name", value)} /><Field label="Provider" value={form.provider} onChange={(value) => update("provider", value)} /><LabeledSelect label="Status" value={form.status} onChange={(value) => update("status", value)} options={[...certificationStatuses]} /><Field label="Target date" type="date" value={form.targetDate} onChange={(value) => update("targetDate", value)} /><Field label="Start date" type="date" value={form.startDate} onChange={(value) => update("startDate", value)} /><Field label="Completion date" type="date" value={form.completionDate} onChange={(value) => update("completionDate", value)} /><Field label="Certificate link" type="url" value={form.certificateLink} onChange={(value) => update("certificateLink", value)} /><Field label="Credential ID" value={form.credentialId} onChange={(value) => update("credentialId", value)} /><label className="min-w-0 md:col-span-2 lg:col-span-3"><span className="mb-1.5 block text-sm font-black">Notes</span><textarea rows={3} value={form.notes} onChange={(event) => update("notes", event.target.value)} className="w-full min-w-0 rounded-2xl border border-slate-300 p-3" /></label></div><button type="submit" className="mt-5 min-h-12 rounded-2xl bg-purple-700 px-6 font-black text-white">{editing ? "Save changes" : "Add certification"}</button></form>{state.certifications.length ? <div className="grid min-w-0 gap-4 md:grid-cols-2 xl:grid-cols-3">{state.certifications.map((item) => <article key={item.id} className="min-w-0 rounded-3xl border border-amber-100 bg-amber-50/60 p-5 shadow-md"><div className="flex min-w-0 items-start justify-between gap-4"><div className="min-w-0"><span className="inline-block max-w-full break-words rounded-full bg-white px-3 py-1 text-xs font-black text-amber-800">{item.status}</span><h2 className="mt-3 break-words text-xl font-black">{item.name}</h2><p className="mt-1 break-words text-sm font-semibold text-slate-600">{item.provider || "Provider not set"}</p></div><div className="flex shrink-0 gap-2"><button type="button" onClick={() => edit(item)} aria-label={`Edit ${item.name}`} className="grid size-11 place-items-center rounded-xl bg-white text-blue-700"><Edit3 size={16} /></button><button type="button" onClick={() => remove(item)} aria-label={`Delete ${item.name}`} className="grid size-11 place-items-center rounded-xl bg-white text-rose-700"><Trash2 size={16} /></button></div></div>{item.notes ? <p className="mt-4 break-words text-sm leading-6 text-slate-700">{item.notes}</p> : null}{item.certificateLink ? <a href={item.certificateLink} target="_blank" rel="noreferrer" className="mt-4 inline-flex max-w-full items-center gap-2 break-words font-black text-purple-800">Certificate <ExternalLink className="shrink-0" size={15} /></a> : null}</article>)}</div> : <EmptyState icon="🏅" text="Add a planned certification when it supports your target role." />}</section>;
}
...
function ProgressCard({ icon, label, value }: { icon: string; label: string; value: string }) { return <article className="min-w-0 rounded-3xl border border-white bg-white/85 p-5 shadow-md"><span className="text-2xl">{icon}</span><p className="mt-3 text-2xl font-black text-purple-900">{value}</p><p className="mt-1 break-words text-sm font-bold text-slate-600">{label}</p></article>; }
function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) { return <div className="min-w-0"><p className="break-words text-sm font-black uppercase tracking-[0.16em] text-purple-700">{eyebrow}</p><h1 className="mt-1 break-words text-3xl font-black">{title}</h1><p className="mt-2 max-w-3xl break-words leading-7 text-slate-700">{description}</p></div>; }
function QuickButton({ icon, label, onClick }: { icon: string; label: string; onClick: () => void }) { return <button type="button" onClick={onClick} className="group flex min-h-28 min-w-0 items-center justify-between rounded-3xl border border-purple-100 bg-white p-5 text-left shadow-md transition hover:-translate-y-1 hover:shadow-xl"><span className="min-w-0"><span className="text-3xl">{icon}</span><span className="mt-2 block break-words font-black">{label}</span></span><ArrowRight className="shrink-0 text-purple-700 transition group-hover:translate-x-1" size={18} /></button>; }
function QuickLink({ icon, label, href }: { icon: string; label: string; href: string }) { return <Link href={href} onClick={playClickSound} className="group flex min-h-28 min-w-0 items-center justify-between rounded-3xl border border-purple-100 bg-white p-5 shadow-md transition hover:-translate-y-1 hover:shadow-xl"><span className="min-w-0"><span className="text-3xl">{icon}</span><span className="mt-2 block break-words font-black">{label}</span></span><ArrowRight className="shrink-0 text-purple-700 transition group-hover:translate-x-1" size={18} /></Link>; }
function EmptyState({ icon, text }: { icon: string; text: string }) { return <div className="min-w-0 rounded-[2rem] border border-dashed border-purple-200 bg-purple-50/50 p-10 text-center"><p className="text-4xl">{icon}</p><p className="mt-3 break-words font-bold text-slate-700">{text}</p></div>; }
function Field({ label, value, onChange, type = "text", required = false, placeholder = "" }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean; placeholder?: string }) { return <label className="min-w-0"><span className="mb-1.5 block break-words text-sm font-black">{label}</span><input type={type} required={required} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="min-h-12 w-full min-w-0 rounded-2xl border border-slate-300 bg-white px-3" /></label>; }
function LabeledSelect({ label, value, onChange, options, labels = {} }: { label: string; value: string; onChange: (value: string) => void; options: readonly string[]; labels?: Record<string, string> }) { return <label className="min-w-0"><span className="sr-only">{label}</span><select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className="min-h-12 w-full min-w-0 rounded-2xl border border-slate-300 bg-white px-3 font-bold">{options.map((option) => <option key={option} value={option}>{labels[option] ?? option}</option>)}</select></label>; }
function Info({ label, value, alert = false }: { label: string; value: string; alert?: boolean }) { return <div className={`min-w-0 rounded-xl p-3 ${alert ? "bg-rose-50 text-rose-800" : "bg-slate-50"}`}><dt className="break-words text-xs font-bold uppercase tracking-wide">{label}</dt><dd className="mt-1 break-words font-semibold">{value}</dd></div>; }
