...
function emptyState(): CareerHubState {
  return { version: 1, completedIds: [], rewardedIds: [], applications: [], certifications: [], favoriteCompanyIds: [], companyNotes: {}, lastSection: "home" };
}
  ...
  return (
    <AppLayout>
      <div className="space-y-7 text-slate-950">
        ...
        {section === "certifications" ? <CertificationTracker state={state} onSync={sync} /> : null}
...
const blankCertification: CertificationInput = { name: "", provider: "", status: "Planned", startDate: "", completionDate: "", certificateLink: "", notes: "" };

function CertificationTracker({ state, onSync }: { state: CareerHubState; onSync: () => void }) {
  ...
  function submit(event: React.FormEvent) { event.preventDefault(); const result = saveCertification({ ...form, name: form.name.trim() }, editing?.id); if (result.saved) { playSuccessSound(); setForm(blankCertification); setEditing(null); onSync(); } }
  function edit(item: CertificationRecord) { playClickSound(); setEditing(item); setForm({ name: item.name, provider: item.provider, status: item.status, startDate: item.startDate, completionDate: item.completionDate, certificateLink: item.certificateLink, notes: item.notes }); }
