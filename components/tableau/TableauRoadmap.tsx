...
import { getTableauLesson } from "@/lib/tableauLessons";
import { tableauRoadmap } from "@/lib/tableauReference";

export default function TableauRoadmap({
  completedLessonIds,
}: {
  completedLessonIds: string[];
}) {
  return (
    <section aria-labelledby="tableau-roadmap-heading">
      <div>
        <p className="text-sm font-bold uppercase tracking-wider text-blue-700">
          Guided path
        </p>
        <h2 id="tableau-roadmap-heading" className="mt-1 text-3xl font-black text-slate-950">
          Learning roadmap
        </h2>
        <p className="mt-2 text-slate-700">
          Move from your first worksheet to production delivery and interview-ready reasoning.
        </p>
      </div>

      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {tableauRoadmap.map((stage) => {
          ...
          const nextId =
            ...
            stage.lessonIds[0];
          const nextLesson = getTableauLesson(nextId);

          return (
            <article
              key={stage.id}
              className="flex flex-col rounded-3xl border border-blue-100 bg-white/85 p-5 shadow-md"
            >
              ...
              {nextLesson ? (
                <Link
                  ...
                  className="mt-4 rounded-xl bg-blue-50 px-4 py-3 text-center text-sm font-bold text-blue-800 transition hover:bg-blue-100"
                >
