...
export default function PythonLessonCard({
  lesson,
  completed,
  favorite,
  onToggleFavorite,
}: PythonLessonCardProps) {
  return (
    <article className="flex h-full flex-col rounded-3xl border border-cyan-100 bg-white/90 p-6 shadow-lg transition duration-200 hover:-translate-y-1 hover:shadow-xl">
      <div className="flex items-start justify-between gap-4">
        <span className="grid size-14 place-items-center rounded-2xl bg-gradient-to-br from-cyan-100 via-blue-100 to-amber-100 text-3xl">
          {lesson.icon}
        </span>
        {completed ? (
      ...
      <div className="mt-5 overflow-hidden rounded-2xl bg-slate-950 p-4">
        <p className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-cyan-200">
          <Code2 size={15} aria-hidden="true" /> Code preview
        </p>
        <pre className="overflow-hidden whitespace-pre-wrap font-mono text-sm leading-6 text-emerald-200">
          <code className="line-clamp-2">{lesson.syntax}</code>
        </pre>
      </div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <Link
          ...
          className="rounded-xl bg-cyan-700 px-4 py-3 text-center font-bold text-white transition hover:bg-cyan-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-700 focus-visible:ring-offset-2"
        >
          Learn
        </Link>
        <Link
