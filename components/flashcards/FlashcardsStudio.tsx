...
function CustomDeckManager({ state, onState, onBack, onOpenDeck }: { state: FlashcardsState; onState: (state: FlashcardsState) => void; onBack: () => void; onOpenDeck: (id: string) => void }) {
  ...
  return <div className="min-w-0 space-y-7 rounded-[2rem] bg-gradient-to-br from-pink-50/70 via-purple-50/65 to-blue-50/70 p-3 text-slate-900 sm:p-5">
    ...
    <section className="grid min-w-0 gap-6 xl:grid-cols-[0.85fr_1.15fr]">
      <form onSubmit={createDeck} className={`${glassSurface} min-w-0 p-6`}><h2 className="text-2xl font-black text-purple-800">Create a deck</h2><div className="mt-5 space-y-4"><Field label="Deck title" value={title} onChange={setTitle} required /><Field label="Description" value={description} onChange={setDescription} /><label className="block font-black text-slate-700">Difficulty<select value={difficulty} onChange={(event) => setDifficulty(event.target.value as FlashcardDifficulty)} className={`mt-2 min-h-12 w-full rounded-2xl px-4 font-semibold ${pastelField}`}>{difficultyOptions.slice(1).map((item) => <option key={item}>{item}</option>)}</select></label><button type="submit" className={`inline-flex min-h-12 items-center gap-2 rounded-2xl bg-purple-700 px-5 font-black text-white shadow-md transition hover:bg-purple-800 ${focusRing}`}><Plus size={18} /> Create deck</button></div></form>
      <section className={`${glassSurface} min-w-0 p-6`}><h2 className="text-2xl font-black text-purple-800">Your decks</h2>{state.customDecks.length ? <div className="mt-5 grid min-w-0 gap-3">{state.customDecks.map((deck) => <article key={deck.id} className={`min-w-0 rounded-2xl border p-4 shadow-sm ${activeDeckId === deck.id ? "border-purple-300 bg-purple-50/90" : "border-white/90 bg-white/75"}`}><button type="button" onClick={() => setActiveDeckId(deck.id)} className={`min-h-11 min-w-0 w-full rounded-xl text-left ${focusRing}`}><p className="break-words font-black text-slate-900">{deck.title}</p><p className="mt-1 text-sm text-slate-600">{getDeckCards(deck.id, state).length} cards · {deck.difficulty}</p></button><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => { const next = window.prompt("Rename deck", deck.title); if (next) onState(renameCustomDeck(deck.id, next).state); }} className={`inline-flex min-h-11 items-center gap-2 rounded-xl border border-sky-200 bg-sky-50 px-3 text-sm font-black text-sky-800 transition hover:bg-sky-100 ${focusRing}`}><Edit3 size={15} /> Rename</button><button type="button" onClick={() => onOpenDeck(deck.id)} className={`min-h-11 rounded-xl border border-purple-200 bg-purple-50 px-3 text-sm font-black text-purple-800 transition hover:bg-purple-100 ${focusRing}`}>Open</button><button type="button" onClick={() => { if (window.confirm(`Delete ${deck.title} and all its cards?`)) { const result = deleteCustomDeck(deck.id); onState(result.state); if (activeDeckId === deck.id) setActiveDeckId(result.state.customDecks[0]?.id ?? ""); } }} className={`inline-flex min-h-11 items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 text-sm font-black text-rose-800 transition hover:bg-rose-100 ${focusRing}`}><Trash2 size={15} /> Delete</button></div></article>)}</div> : <p className="mt-5 rounded-2xl border border-dashed border-purple-200 bg-purple-50/50 p-8 text-center font-bold text-slate-600">Create your first personal deck.</p>}</section>
    </section>
    {activeDeck ? <section className="grid min-w-0 gap-6 xl:grid-cols-[0.85fr_1.15fr]"><form onSubmit={saveCard} className={`${glassSurface} min-w-0 bg-gradient-to-br from-white/80 via-sky-50/75 to-purple-50/70 p-6`}><h2 className="break-words text-2xl font-black text-purple-800">{editingCardId ? "Edit card" : `Add to ${activeDeck.title}`}</h2><div className="mt-5 space-y-4"><TextArea label="Front" value={front} onChange={setFront} required /><TextArea label="Back" value={back} onChange={setBack} required /><label className="block font-black text-slate-700">Difficulty<select value={cardDifficulty} onChange={(event) => setCardDifficulty(event.target.value as FlashcardDifficulty)} className={`mt-2 min-h-12 w-full rounded-2xl px-4 font-semibold ${pastelField}`}>{difficultyOptions.slice(1).map((item) => <option key={item}>{item}</option>)}</select></label><div className="flex flex-wrap gap-3"><button type="submit" className={`inline-flex min-h-12 items-center gap-2 rounded-2xl bg-sky-700 px-5 font-black text-white shadow-md transition hover:bg-sky-800 ${focusRing}`}><Check size={18} /> {editingCardId ? "Save changes" : "Add card"}</button>{editingCardId ? <button type="button" onClick={() => { setEditingCardId(undefined); setFront(""); setBack(""); }} className={`min-h-12 rounded-2xl border border-purple-200 bg-white/80 px-5 font-black text-slate-700 shadow-sm transition hover:bg-purple-50 ${focusRing}`}>Cancel</button> : null}</div></div></form><section className={`${glassSurface} min-w-0 p-6`}><h2 className="text-2xl font-black text-purple-800">Cards</h2>{activeCards.length ? <div className="mt-5 min-w-0 space-y-3">{activeCards.map((card) => <article key={card.id} className="min-w-0 rounded-2xl border border-white/90 bg-gradient-to-br from-white/85 to-purple-50/65 p-4 shadow-sm"><p className="break-words font-black text-slate-900">{card.front}</p><p className="mt-2 break-words text-sm leading-6 text-slate-700">{card.back}</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => editCard(card)} className={`inline-flex min-h-11 items-center gap-2 rounded-xl border border-sky-200 bg-sky-50 px-3 text-sm font-black text-sky-800 transition hover:bg-sky-100 ${focusRing}`}><Edit3 size={15} /> Edit</button><button type="button" onClick={() => { if (window.confirm("Delete this card?")) onState(deleteCustomFlashcard(card.id).state); }} className={`inline-flex min-h-11 items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 text-sm font-black text-rose-800 transition hover:bg-rose-100 ${focusRing}`}><Trash2 size={15} /> Delete</button></div></article>)}</div> : <p className="mt-5 rounded-2xl border border-dashed border-sky-200 bg-sky-50/50 p-8 text-center font-bold text-slate-600">Add the first card to this deck.</p>}</section></section> : null}
  </div>;
}
