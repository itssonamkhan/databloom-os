...
import { getPowerQueryPracticeDataset } from "@/lib/powerQueryPracticeDatasets";
import {
  completePowerQueryPractice,
  isAcceptedPowerQueryAnswer,
...
export default function PowerQueryPractice({
  lesson,
  nextLesson,
}: {
  lesson: PowerQueryLesson;
  nextLesson?: PowerQueryLesson;
}) {
  ...
  const [completed, setCompleted] = useState(() =>
    loadPowerQueryProgress().completedPracticeIds.includes(lesson.id),
  );
  const reward = Math.max(10, Math.floor(lesson.xpReward / 2));
  const dataset = getPowerQueryPracticeDataset(lesson.category, lesson.id);
  ...
  function checkAnswer() {
    ...
    if (!answer) {
      setFeedback({
        tone: "warning",
        message: "Choose an answer before checking this Power Query scenario.",
      });
      return;
    }

    if (!isAcceptedPowerQueryAnswer(answer, lesson.correctAnswer)) {
      playNotificationSound();
      setFeedback({
        tone: "incorrect",
        message: "Not quite yet. Focus on a repeatable transformation, use the hint, and try again.",
      });
      return;
    }

    const result = completePowerQueryPractice(lesson.id);
    ...
    <AppLayout>
      <div className="space-y-7 text-slate-950">
        ...
        <header className="rounded-3xl border border-white/70 bg-gradient-to-br from-teal-100 via-blue-100 to-purple-100 p-7 shadow-lg sm:p-10">
          <p className="font-bold text-teal-900">Power Query Practice · {lesson.difficulty}</p>
          <h1 className="mt-2 text-4xl font-black sm:text-5xl">
            {lesson.icon} {lesson.title}
          </h1>
          <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-700">
            Use the {dataset.title.toLocaleLowerCase()} below and choose the transformation outcome that best supports a reliable refresh.
          </p>
          <div className="mt-5 flex flex-wrap gap-3 text-sm font-bold">
            <span className="rounded-full bg-white/85 px-4 py-2 text-teal-900">+{reward} XP once</span>
            {completed ? (
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h2 className="flex items-center gap-2 text-2xl font-black">
                <Table2 className="text-teal-800" aria-hidden="true" /> {dataset.title}
              </h2>
              <p className="mt-2 max-w-3xl leading-7 text-slate-700">{dataset.description}</p>
              <p className="mt-3 max-w-3xl rounded-2xl bg-teal-50 p-4 font-semibold leading-7 text-teal-950">
            ...
            <div className="flex flex-wrap gap-2">
              {dataset.downloads.map((download) => (
                <a
                  ...
                  download
                  onClick={playClickSound}
                  className="inline-flex items-center gap-2 rounded-xl border border-teal-200 bg-teal-50 px-4 py-3 font-bold text-teal-900 transition hover:bg-teal-100"
                >
                  <Download size={18} aria-hidden="true" /> {download.label}
                </a>
              ))}
            </div>
          </div>
                ...
                {dataset.rows.map((row, rowIndex) => (
                  <tr key={`${dataset.id}-${rowIndex}`}>
                    {row.map((cell, columnIndex) => (
                      <td key={`${dataset.id}-${rowIndex}-${dataset.columns[columnIndex]}`} className="whitespace-nowrap px-4 py-3 text-slate-700">
                        {cell}
                      </td>
                    ))}
                  </tr>
        ...
        <section className="rounded-3xl border border-purple-100 bg-white p-6 shadow-lg sm:p-8">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-purple-300 bg-purple-50 px-5 py-3 font-bold text-purple-900 transition hover:bg-purple-100"
            >
          ...
          {showHint ? (
            <div className="mt-4 rounded-2xl border border-purple-200 bg-purple-50 p-4 leading-7 text-purple-950">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <Link
            href={`/power-query-studio/${lesson.id}`}
            onClick={playClickSound}
            className="rounded-xl border border-teal-200 bg-white px-5 py-3 font-bold text-teal-900 shadow-sm"
          >
          ...
          {nextLesson ? (
            <Link
              ...
              onClick={playClickSound}
              className="inline-flex items-center gap-2 rounded-xl bg-purple-700 px-5 py-3 font-bold text-white transition hover:bg-purple-800"
            >
