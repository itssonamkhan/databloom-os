...
function TemplatePicker({ onChoose, onClose }: { onChoose: (id: string) => void; onClose: () => void }) {
  return (
    <section aria-labelledby="template-heading" className="rounded-[2rem] border border-purple-100 bg-white/90 p-6 shadow-lg">
      <div className="flex min-w-0 items-start justify-between gap-4"><div className="min-w-0"><p className="text-sm font-black uppercase tracking-wider text-purple-500">Start with a structure</p><h2 id="template-heading" className="mt-1 break-words text-2xl font-black text-slate-900">Choose a note template</h2></div><button type="button" onClick={onClose} className={`min-h-11 shrink-0 rounded-xl px-3 py-2 font-bold text-slate-600 hover:bg-slate-100 ${buttonFocus}`}>Close</button></div>
  ...
  return (
    <>
      ...
      <section aria-labelledby="search-notes-heading" className="rounded-[2rem] border border-white/90 bg-white/80 p-5 shadow-md">
        ...
        {state.recentSearches.length ? <div className="mt-4 flex flex-wrap items-center gap-2"><span className="text-xs font-black uppercase tracking-wider text-slate-500">Recent searches</span>{state.recentSearches.map((item) => <button key={item} type="button" onClick={() => onRecentSearch(item)} className={`min-h-11 rounded-full bg-purple-50 px-3 py-1.5 text-xs font-bold text-purple-700 ${buttonFocus}`}>{item}</button>)}</div> : null}
      </section>
...
function NoteCard({ note, onOpen, onPatch }: { note: SmartNote; onOpen: (id: string) => void; onPatch: (id: string, patch: Partial<SmartNote>) => void }) {
  return <article className="group min-w-0 rounded-3xl border border-purple-100 bg-gradient-to-br from-white to-pink-50 p-4 shadow-sm"><div className="flex min-w-0 items-start justify-between gap-2"><button type="button" onClick={() => onOpen(note.id)} className={`min-w-0 flex-1 text-left ${buttonFocus}`}><span className="block break-words text-lg font-black text-purple-900">{note.title || "Untitled note"}</span><span className="mt-1 block break-words text-xs font-bold text-slate-500">{note.subject} · {formatDate(note.updatedAt)}</span></button><div className="flex shrink-0 gap-1"><IconButton label={note.favorite ? "Remove favorite" : "Favorite note"} active={note.favorite} onClick={() => onPatch(note.id, { favorite: !note.favorite })}><Heart size={16} fill={note.favorite ? "currentColor" : "none"} /></IconButton><IconButton label={note.pinned ? "Unpin note" : "Pin note"} active={note.pinned} onClick={() => onPatch(note.id, { pinned: !note.pinned })}><Pin size={16} fill={note.pinned ? "currentColor" : "none"} /></IconButton></div></div><button type="button" onClick={() => onOpen(note.id)} className={`mt-4 block w-full text-left ${buttonFocus}`}><span className="line-clamp-3 whitespace-pre-wrap text-sm leading-6 text-slate-600">{note.content || "A clean page waiting for your ideas."}</span><span className="mt-4 inline-flex items-center gap-1 text-xs font-black text-purple-700">Open note <ChevronRight size={14} /></span></button></article>;
}

function ActivityColumn({ title, notes, onOpen, empty }: { title: string; notes: SmartNote[]; onOpen: (id: string) => void; empty: string }) {
  return <div className="min-w-0 rounded-3xl bg-gradient-to-br from-purple-50 to-sky-50 p-4"><h3 className="font-black text-purple-900">{title}</h3><div className="mt-3 space-y-2">{notes.length ? notes.slice(0, 3).map((note) => <button key={note.id} type="button" onClick={() => onOpen(note.id)} className={`flex min-h-11 min-w-0 w-full items-center justify-between gap-3 rounded-2xl bg-white/85 px-4 text-left text-sm font-bold text-slate-700 shadow-sm ${buttonFocus}`}><span className="min-w-0 break-words">{note.title}</span><ChevronRight size={15} className="shrink-0 text-purple-600" /></button>) : <p className="rounded-2xl border border-dashed border-purple-200 p-4 text-sm text-slate-500">{empty}</p>}</div></div>;
}
...
function CollectionsView({ state, name, description, onName, onDescription, onSubmit, onOpen, onDelete }: { state: SmartNotesState; name: string; description: string; onName: (value: string) => void; onDescription: (value: string) => void; onSubmit: (event: React.FormEvent) => void; onOpen: (id: string) => void; onDelete: (id: string) => void }) {
  return <section className="grid min-w-0 gap-6 xl:grid-cols-[22rem_minmax(0,1fr)]"><form onSubmit={onSubmit} className="min-w-0 h-fit rounded-[2rem] border border-white bg-white/80 p-6 shadow-md"><FolderPlus className="text-emerald-700" /><h2 className="mt-3 text-2xl font-black text-slate-900">New study collection</h2><p className="mt-2 text-sm leading-6 text-slate-600">Group notes for a course, project, or interview goal.</p><div className="mt-5 space-y-3"><MetaInput label="Collection name" value={name} onChange={onName} /><MetaInput label="Description" value={description} onChange={onDescription} /><button type="submit" className={`min-h-11 w-full rounded-2xl bg-emerald-700 px-5 font-black text-white ${buttonFocus}`}>Create collection</button></div></form><div className="min-w-0 space-y-4">{state.collections.length ? state.collections.map((collection) => { const notes = state.notes.filter((note) => note.collectionId === collection.id && !note.trashed); return <article key={collection.id} className="min-w-0 rounded-[2rem] border border-white bg-gradient-to-br from-emerald-50 to-sky-50 p-6 shadow-sm"><div className="flex min-w-0 items-start justify-between gap-4"><div className="min-w-0"><h2 className="break-words text-2xl font-black text-slate-900">📚 {collection.name}</h2><p className="mt-1 break-words text-sm text-slate-600">{collection.description || `${notes.length} grouped note${notes.length === 1 ? "" : "s"}`}</p></div><button type="button" onClick={() => onDelete(collection.id)} className={`grid size-11 shrink-0 place-items-center rounded-xl text-rose-600 hover:bg-rose-100 ${buttonFocus}`} aria-label={`Delete ${collection.name} collection`}><Trash2 size={18} /></button></div><div className="mt-4 grid min-w-0 gap-3 sm:grid-cols-2">{notes.length ? notes.map((note) => <button type="button" key={note.id} onClick={() => onOpen(note.id)} className={`min-w-0 break-words rounded-2xl bg-white/85 p-4 text-left font-black text-purple-800 shadow-sm ${buttonFocus}`}>{note.title}</button>) : <p className="rounded-2xl border border-dashed border-emerald-200 p-5 text-sm text-slate-600">Add notes to this collection from the editor.</p>}</div></article>; }) : <div className="rounded-[2rem] border border-dashed border-purple-200 bg-purple-50/50 p-10 text-center"><p className="text-4xl">📚</p><h2 className="mt-3 text-xl font-black text-slate-900">No collections yet</h2><p className="mt-2 text-sm text-slate-600">Your first focused study set starts here.</p></div>}</div></section>;
}

function TrashView({ notes, onRestore, onDelete }: { notes: SmartNote[]; onRestore: (note: SmartNote) => void; onDelete: (id: string) => void }) {
  return <section className="min-w-0 rounded-[2rem] border border-white bg-white/80 p-6 shadow-md"><h2 className="text-2xl font-black text-slate-900">Trash</h2><p className="mt-1 text-sm text-slate-600">Restore notes or delete them permanently.</p>{notes.length ? <div className="mt-5 grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-3">{notes.map((note) => <article key={note.id} className="min-w-0 rounded-3xl border border-rose-100 bg-rose-50/60 p-5"><h3 className="break-words font-black text-slate-900">{note.title}</h3><p className="mt-1 text-xs text-slate-500">Edited {formatDate(note.updatedAt)}</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => onRestore(note)} className={`min-h-11 rounded-xl bg-white px-3 py-2 text-sm font-bold text-emerald-700 ${buttonFocus}`}>Restore</button><button type="button" onClick={() => onDelete(note.id)} className={`min-h-11 rounded-xl bg-rose-700 px-3 py-2 text-sm font-bold text-white ${buttonFocus}`}>Delete forever</button></div></article>)}</div> : <div className="mt-5 rounded-3xl border border-dashed border-slate-200 p-10 text-center text-sm font-medium text-slate-600">Trash is empty. Your notes are safe and sound.</div>}</section>;
}
