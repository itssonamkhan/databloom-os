...
function StatusGroup({ title, icon, items, empty, showDate = false }: { title: string; icon: string; items: CertificationRecord[]; empty: string; showDate?: boolean }) {
  return <article className="min-w-0 rounded-[1.75rem] border border-white/90 bg-white/75 p-5 shadow-md backdrop-blur-xl"><p className="text-2xl" aria-hidden="true">{icon}</p><h3 className="mt-3 text-xl font-black text-purple-900">{title}</h3>{items.length ? <ul className="mt-4 space-y-3">{items.slice(0, 4).map((item) => <li key={item.id} className="min-w-0 rounded-2xl border border-purple-100 bg-purple-50/60 p-3"><p className="break-words font-black text-slate-900">{item.name}</p><p className="mt-1 break-words text-xs font-semibold text-slate-600">{showDate ? item.targetDate : item.provider || item.status}</p></li>)}</ul> : <p className="mt-4 text-sm leading-6 text-slate-600">{empty}</p>}</article>;
}

function RecommendationCard({ item, onAdd }: { item: CertificationDefinition; onAdd: () => void }) {
  return <article className="min-w-0 rounded-[1.75rem] border border-purple-100 bg-gradient-to-br from-white/90 via-pink-50/80 to-purple-50/80 p-5 shadow-lg"><div className="flex min-w-0 flex-wrap items-center justify-between gap-3"><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-purple-800">{item.provider}</span><span className="text-xs font-black text-emerald-700">{item.difficulty}</span></div><h3 className="mt-4 break-words text-xl font-black text-slate-950">{item.name}</h3><p className="mt-2 break-words text-sm leading-6 text-slate-600">{item.summary}</p><div className="mt-5 flex flex-wrap gap-3"><Link href={`/certification-hub/${item.id}`} onClick={playClickSound} className={`inline-flex min-h-11 items-center gap-2 rounded-2xl bg-purple-700 px-4 font-black text-white ${focusClass}`}>View roadmap <ArrowRight size={16} /></Link><button type="button" onClick={onAdd} className={`min-h-11 rounded-2xl border border-purple-200 bg-white px-4 font-black text-purple-800 ${focusClass}`}>Add to tracker</button></div></article>;
}
...
function TrackerCard({ item, state, onEdit, onDelete }: { item: CertificationRecord; state: CareerHubState; onEdit: () => void; onDelete: () => void }) {
  ...
  return <article className="min-w-0 rounded-[1.75rem] border border-purple-100 bg-white/80 p-5 shadow-md"><div className="flex min-w-0 items-start justify-between gap-4"><div className="min-w-0"><span className="rounded-full bg-purple-100 px-3 py-1 text-xs font-black text-purple-800">{item.status}</span><h3 className="mt-3 break-words text-xl font-black text-slate-950">{item.name}</h3><p className="mt-1 break-words text-sm font-semibold text-slate-600">{item.provider || "Provider not set"}</p></div><div className="flex shrink-0 gap-2"><button type="button" onClick={onEdit} aria-label={`Edit ${item.name}`} className={`grid size-11 place-items-center rounded-xl border border-sky-200 bg-sky-50 text-sky-800 ${focusClass}`}><Edit3 size={16} /></button><button type="button" onClick={onDelete} aria-label={`Delete ${item.name}`} className={`grid size-11 place-items-center rounded-xl border border-rose-200 bg-rose-50 text-rose-800 ${focusClass}`}><Trash2 size={16} /></button></div></div><div className="mt-4"><div className="flex items-center justify-between text-sm font-bold text-slate-700"><span>Readiness</span><span>{progress.percentage}%</span></div><div className="mt-2 h-2.5 overflow-hidden rounded-full bg-purple-100"><div className="h-full rounded-full bg-gradient-to-r from-purple-600 to-pink-500" style={{ width: `${progress.percentage}%` }} /></div></div><dl className="mt-4 grid min-w-0 grid-cols-2 gap-3"><Info label="Target" value={item.targetDate || "Not set"} /><Info label="Started" value={item.startDate || "Not set"} /><Info label="Completed" value={item.completionDate || "Not set"} /><Info label="Credential ID" value={item.credentialId || "Not set"} /></dl>{item.notes ? <p className="mt-4 break-words rounded-2xl bg-slate-50 p-3 text-sm leading-6 text-slate-700">{item.notes}</p> : null}<div className="mt-4 flex flex-wrap gap-3">{item.catalogId ? <Link href={`/certification-hub/${item.catalogId}`} onClick={playClickSound} className="inline-flex items-center gap-2 font-black text-purple-800">Preparation guide <ArrowRight size={16} /></Link> : null}{item.certificateLink ? <a href={item.certificateLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 font-black text-sky-800">Certificate <ExternalLink size={15} /></a> : null}</div></article>;
}
