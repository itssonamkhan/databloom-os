    ...
    <AppLayout>
      <div className="min-w-0 space-y-8 text-slate-950">
        ...
        <section className="grid min-w-0 gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <article className="min-w-0 rounded-[2rem] border border-purple-200 bg-gradient-to-br from-purple-50 via-white to-pink-50 p-6 shadow-lg sm:p-8">
          ...
          <article className="min-w-0 rounded-[2rem] border border-emerald-200 bg-white/85 p-6 shadow-lg sm:p-8">
        ...
        <section>
          ...
          <div className="mt-5 grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {interviewCategories.map((item) => {
              ...
              return <button key={item} type="button" onClick={() => { playClickSound(); setCategory(item); setPage(1); document.getElementById("question-library")?.scrollIntoView({ behavior: "smooth" }); }} className={`min-w-0 rounded-3xl border bg-gradient-to-br p-5 text-left shadow-md transition hover:-translate-y-1 hover:shadow-xl ${meta.className}`}><span className="text-3xl">{meta.icon}</span><h3 className="mt-3 break-words text-xl font-black">{item}</h3><p className="mt-2 min-h-12 text-sm leading-6 text-slate-700">{meta.description}</p><p className="mt-3 text-sm font-black text-purple-800">{learned}/{total} learned</p></button>;
            })}
          </div>
        </section>

        <section id="question-library" className="min-w-0 rounded-[2rem] border border-purple-100 bg-white/80 p-5 shadow-lg sm:p-7">
          ...
          {visibleQuestions.length ? <div className="mt-6 grid min-w-0 gap-4 md:grid-cols-2 xl:grid-cols-3">{visibleQuestions.map((question) => <QuestionCard key={question.id} question={question} learned={state.learnedQuestionIds.includes(question.id)} favorite={state.favoriteQuestionIds.includes(question.id)} onOpen={() => openQuestion(question)} onFavorite={() => { playClickSound(); toggleInterviewFavorite(question.id); sync(); }} />)}</div> : <EmptyPanel text="No interview questions match these filters." />}
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-bold text-slate-600">Page {page} of {pageCount}</p><div className="flex gap-2"><button type="button" disabled={page === 1} onClick={() => { setPage((current) => Math.max(1, current - 1)); window.scrollTo({ top: document.getElementById("question-library")?.offsetTop ?? 0, behavior: "smooth" }); }} className="min-h-11 rounded-xl border border-purple-200 bg-white px-4 py-2 font-black text-purple-800 disabled:opacity-40">Previous</button><button type="button" disabled={page === pageCount} onClick={() => { setPage((current) => Math.min(pageCount, current + 1)); window.scrollTo({ top: document.getElementById("question-library")?.offsetTop ?? 0, behavior: "smooth" }); }} className="min-h-11 rounded-xl bg-purple-700 px-4 py-2 font-black text-white disabled:opacity-40">Next</button></div></div>
        </section>
        ...
        <section className="rounded-[2rem] border border-amber-100 bg-white/85 p-6 shadow-lg sm:p-8">
          ...
          <div className="mt-6 grid min-w-0 gap-5 lg:grid-cols-2 xl:grid-cols-3">
            {resumeReviewSections.map((section) => <article key={section.id} className="min-w-0 rounded-3xl border border-amber-100 bg-amber-50/50 p-5"><div className="flex min-w-0 items-center gap-3"><span className="text-2xl">{section.icon}</span><h3 className="min-w-0 break-words text-xl font-black">{section.title}</h3></div><div className="mt-4 space-y-3">{section.items.map((label, index) => { const id = `${section.id}-${index + 1}`; const checked = state.checklistCompletedIds.includes(id); return <label key={id} className="flex min-w-0 cursor-pointer items-start gap-3 rounded-2xl border border-white bg-white/75 p-3"><input type="checkbox" checked={checked} onChange={() => { playClickSound(); toggleResumeChecklistItem(id); sync(); }} className="mt-1 size-4 shrink-0 accent-emerald-600" /><span className={`min-w-0 break-words text-sm font-semibold leading-6 ${checked ? "text-slate-500 line-through" : "text-slate-800"}`}>{label}</span></label>; })}</div></article>)}
          </div>
        </section>
      </div>
    </AppLayout>
...
function QuestionDetail({ question, learned, favorite, note, onBack, onLearn, onFavorite, onSaveNote }: { question: InterviewQuestion; learned: boolean; favorite: boolean; note: string; onBack: () => void; onLearn: () => void; onFavorite: () => void; onSaveNote: (note: string) => void }) {
  ...
  return <div className="min-w-0 space-y-6 text-slate-950"><button type="button" onClick={onBack} className="inline-flex items-center gap-2 font-black text-purple-800"><ArrowLeft size={18} /> Back to Interview Hub</button><header className="min-w-0 rounded-[2rem] border border-white bg-gradient-to-br from-purple-100 via-pink-100 to-sky-100 p-7 shadow-lg sm:p-9"><div className="flex min-w-0 flex-wrap items-start justify-between gap-5"><div className="min-w-0 max-w-4xl"><div className="flex min-w-0 flex-wrap gap-2 text-sm font-bold"><Pill>{question.category}</Pill><Pill>{question.topic}</Pill><Pill>{question.difficulty}</Pill><Pill>+{question.xpReward} XP once</Pill></div><h1 className="mt-5 break-words text-3xl font-black leading-tight sm:text-4xl">{question.question}</h1></div><button type="button" onClick={onFavorite} aria-pressed={favorite} className={`grid size-12 shrink-0 place-items-center rounded-2xl border shadow-sm ${favorite ? "border-pink-300 bg-pink-100 text-pink-700" : "border-white bg-white/80 text-slate-500"}`} aria-label={favorite ? "Remove saved question" : "Save question"}><Heart size={21} fill={favorite ? "currentColor" : "none"} /></button></div><button type="button" onClick={onLearn} className={`mt-6 inline-flex min-h-12 items-center gap-2 rounded-2xl px-6 font-black text-white ${learned ? "bg-emerald-700" : "bg-purple-700"}`}><CheckCircle2 size={19} /> {learned ? "Learned · review again" : `Mark learned · +${question.xpReward} XP`}</button></header><div className="grid min-w-0 gap-6 xl:grid-cols-2"><AnswerPanel icon={<MessageSquareText />} title="Ideal answer" tone="purple"><p>{question.idealAnswer}</p></AnswerPanel><AnswerPanel icon={<BookOpenCheck />} title="Beginner explanation" tone="emerald"><p>{question.beginnerExplanation}</p></AnswerPanel><AnswerPanel icon={<Sparkles />} title="Advanced explanation" tone="blue"><p>{question.advancedExplanation}</p></AnswerPanel><AnswerPanel icon={<Lightbulb />} title="Real interview tip" tone="amber"><p>{question.realInterviewTip}</p></AnswerPanel><AnswerPanel icon={<RotateCcw />} title="Common mistakes" tone="rose"><ul className="list-disc space-y-2 pl-5">{question.commonMistakes.map((item) => <li key={item}>{item}</li>)}</ul></AnswerPanel><AnswerPanel icon={<ArrowRight />} title="Follow-up questions" tone="sky"><ul className="space-y-3">{question.followUpQuestions.map((item) => <li key={item} className="rounded-xl bg-white/70 p-3 font-semibold">{item}</li>)}</ul></AnswerPanel></div><section className="min-w-0 rounded-[2rem] border border-pink-100 bg-white/85 p-6 shadow-md"><label className="block min-w-0"><span className="mb-2 block font-black">Personal notes</span><textarea value={draft} onChange={(event) => { setDraft(event.target.value); setSaved(""); }} rows={5} placeholder="Save an example, talking point, or revision note…" className="max-w-full w-full rounded-2xl border border-slate-300 p-4 outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-100" /></label><button type="button" onClick={() => { playClickSound(); onSaveNote(draft); setSaved("Note saved on this device."); }} className="mt-3 min-h-11 rounded-xl bg-purple-700 px-5 py-3 font-black text-white">Save note</button><p className="mt-2 min-h-5 text-sm font-bold text-emerald-700" aria-live="polite">{saved}</p></section></div>;
}
...
function HomeList({ title, eyebrow, icon, empty, questions, state, onOpen }: { title: string; eyebrow: string; icon: React.ReactNode; empty: string; questions: InterviewQuestion[]; state: InterviewHubState; onOpen: (question: InterviewQuestion) => void }) { return <article className="min-w-0 max-w-full rounded-[2rem] border border-purple-100 bg-white/85 p-6 shadow-lg"><div className="flex min-w-0 items-center gap-3"><div className="grid size-10 shrink-0 place-items-center rounded-2xl bg-purple-100 text-purple-700">{icon}</div><div className="min-w-0"><p className="text-xs font-black uppercase tracking-wider text-purple-700">{eyebrow}</p><h2 className="break-words text-xl font-black">{title}</h2></div></div><div className="mt-5 min-w-0 space-y-3">{questions.length ? questions.map((question) => <button key={question.id} type="button" onClick={() => onOpen(question)} className="flex min-w-0 w-full items-center gap-3 rounded-2xl border border-purple-100 bg-purple-50/50 p-3 text-left transition hover:bg-purple-100"><span className="shrink-0 text-xl">{categoryMeta[question.category].icon}</span><div className="min-w-0 flex-1"><p className="truncate font-black">{question.question}</p><p className="mt-1 truncate text-xs font-semibold text-slate-600">{question.topic} · {question.difficulty}</p></div>{state.learnedQuestionIds.includes(question.id) ? <Check size={17} className="shrink-0 text-emerald-700" /> : <ArrowRight size={17} className="shrink-0 text-purple-700" />}</button>) : <EmptyPanel text={empty} compact />}</div></article>; }

function QuestionCard({ question, learned, favorite, onOpen, onFavorite }: { question: InterviewQuestion; learned: boolean; favorite: boolean; onOpen: () => void; onFavorite: () => void }) { return <article className="flex min-h-64 min-w-0 max-w-full flex-col rounded-3xl border border-purple-100 bg-white p-5 shadow-sm"><div className="flex min-w-0 items-start justify-between gap-3"><div className="flex min-w-0 flex-wrap gap-2 text-xs font-bold"><span className="max-w-full break-words rounded-full bg-purple-100 px-3 py-1 text-purple-800">{question.topic}</span><span className="rounded-full bg-slate-100 px-3 py-1 text-slate-700">{question.difficulty}</span></div><button type="button" onClick={onFavorite} aria-label={favorite ? "Remove saved question" : "Save question"} className="grid size-11 shrink-0 place-items-center rounded-xl text-pink-600"><Heart size={19} fill={favorite ? "currentColor" : "none"} /></button></div><h3 className="mt-4 flex-1 break-words text-lg font-black leading-7">{question.question}</h3><div className="mt-5 flex flex-wrap items-center justify-between gap-3"><span className={`text-sm font-black ${learned ? "text-emerald-700" : "text-amber-700"}`}>{learned ? "Learned" : `+${question.xpReward} XP`}</span><button type="button" onClick={onOpen} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-purple-700 px-4 py-2 text-sm font-black text-white">Open answer</button></div></article>; }

function AnswerPanel({ icon, title, tone, children }: { icon: React.ReactNode; title: string; tone: "purple" | "emerald" | "blue" | "amber" | "rose" | "sky"; children: React.ReactNode }) { const colors = { purple: "border-purple-100 bg-purple-50/60 text-purple-800", emerald: "border-emerald-100 bg-emerald-50/60 text-emerald-800", blue: "border-blue-100 bg-blue-50/60 text-blue-800", amber: "border-amber-100 bg-amber-50/60 text-amber-800", rose: "border-rose-100 bg-rose-50/60 text-rose-800", sky: "border-sky-100 bg-sky-50/60 text-sky-800" }; return <section className={`min-w-0 max-w-full rounded-[2rem] border p-6 shadow-sm ${colors[tone]}`}><div className="flex min-w-0 items-center gap-3"><div className="grid size-10 shrink-0 place-items-center rounded-2xl bg-white/80">{icon}</div><h2 className="min-w-0 break-words text-xl font-black">{title}</h2></div><div className="mt-4 break-words leading-7 text-slate-700">{children}</div></section>; }
