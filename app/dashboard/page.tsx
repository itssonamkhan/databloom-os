...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const syncFlashcardsProgress = () =>
      setFlashcardsSummary(getFlashcardsSummary(loadFlashcardsState()));
    window.addEventListener(FLASHCARDS_EVENT, syncFlashcardsProgress);
    return () =>
      window.removeEventListener(FLASHCARDS_EVENT, syncFlashcardsProgress);
  }, []);
      ...
      <div className="space-y-8">
        ...
        <section aria-labelledby="skills-heading">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-12">
            ...
            <SkillProgress
              title="🧠 Flashcards"
              progress={flashcardsSummary.progressPercentage}
            />
            <SkillProgress
        ...
        <section className="rounded-3xl border border-white/70 bg-white/80 p-6 shadow-lg sm:p-8">
          ...
          <div className="mt-5 flex flex-wrap gap-4">
            ...
            <Link href="/flashcards" onClick={playClickSound} className="rounded-xl bg-pink-700 px-5 py-3 font-bold text-white transition hover:bg-pink-800">🧠 Review Flashcards</Link>
            <Link href="/certification-hub" onClick={playClickSound} className="rounded-xl bg-amber-700 px-5 py-3 font-bold text-white transition hover:bg-amber-800">🏅 Certification Hub</Link>
            <Link
              href="/python-studio"
              onClick={playClickSound}
              className="rounded-xl bg-emerald-700 px-5 py-3 font-bold text-white transition hover:bg-emerald-800"
